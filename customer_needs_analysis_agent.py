...
# =========================================
print("⬆️  Upload a PDF, DOCX, or TXT version of the RFP")
...
print(f"✅ Loaded {len(rfp_text):,} characters of RFP text")


MAX_CHARS = 18_000
if len(rfp_text) > MAX_CHARS:
    print("⚠️  RFP is long – truncating to first", MAX_CHARS, "chars")
...
# =========================================
SYSTEM_INSTRUCTION = """
You are a seasoned pre‑sales consultant. Given an RFP, you must extract and
synthesize the CLIENT’S NEEDS, PAIN POINTS, and SUCCESS CRITERIA.
...
3. Implicit Wish‑List (features not explicitly asked for but strongly implied)
...
try:
    doc.build(flow)
    print(f"✅ Well-formatted PDF saved → {pdf_path}")
    ...
    doc.build(flow)
    print(f"✅ PDF saved with basic formatting → {pdf_path}")
    files.download(pdf_path)
